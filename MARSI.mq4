...
int maxTradesOverall = 5;
int order;

input double RiskRatio=0.010; //valeur par defaut de perte max stoploss
input double RiskReward=1.5;// valeur 

input int Inpstoploss= 400; // risque en pips pour le Stoploss
...
void closeTradesOnProfit() {
   ...
   if (totalProfit >= 0.025*AccountBalance()|| totalProfit <= -0.02*AccountBalance()) {
      for (int p = 0; p < totalTrades; p++) {
         if (OrderSelect(p, SELECT_BY_POS, MODE_TRADES)) {
            if (OrderSymbol() == Symbol()) {
